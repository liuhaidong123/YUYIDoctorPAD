...
public class RongDbUtlis {
    static RongDbUtlis dbUtils;
    RongDbHelper helper;
    private RongDbUtlis(){
    }
    public static RongDbUtlis getInstance(){
        if (dbUtils==null){
            dbUtils=new RongDbUtlis();
        }
        return dbUtils;
    }
    ...
    public String getOkhttpString(Context context, String url){
        helper=new RongDbHelper(context, "rong.db", null, 1);
